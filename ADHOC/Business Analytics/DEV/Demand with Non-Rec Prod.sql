...
,HoldingKey
, CASE
WHEN (FunctionName in('Operations 1st Notice', 'Life Proofs', 'Life Complex', 'Bene Admin BAU', 'Life 2nd Exam', 'Life Other', 'Life Holds') OR (FunctionName = 'Phone Claims' AND WorkEventName in ('{LC} TC Live Claim Call', '{LC} TC OUTBOUND CLAIM', '{LC} Triage-TC Outbound Claim')) OR (FunctionName = 'Life Follow Ups' AND SegmentName <> 'Life Follow Ups RM'))  THEN 'Life Claim Examiner'
...
,CASE WHEN "WorkFunction" = 'Operations 1st Notice' THEN SUM(28.50/60.00)
...
SELECT	
'Meeting' as "TransactionTypeName",
NULL AS "ForecastID",
NULL AS "SourceTransactionID",
CAST (NULL AS  VARCHAR (40)) AS "HoldingKey", 
...
Duration AS "Demand",
CAST(NULL AS INTEGER) AS "ForcastVolume_high95",
CAST(NULL AS INTEGER) AS "ForecastVolume_high80",
CAST(NULL AS INTEGER) AS "ForecastVolume_low80",
CAST(NULL AS INTEGER) AS "ForecastVolume_low95",
CAST(NULL AS INTEGER) AS "ForecastDemand_high95",
