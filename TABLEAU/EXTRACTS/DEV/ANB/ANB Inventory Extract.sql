SELECT T1.INVENTORYID 
 ...
 ,ReportDate   
FROM DEV_DMA_VW.ANB_INVENTORY_FCT_VW T1
LEFT JOIN DEV_DMA_VW.TRANSACTION_TYPE_LOV_VW T2 ON T1.TRANSACTIONTYPEID = T2.TRANSACTIONTYPEID
LEFT JOIN DEV_DMA_VW.NB_APPL_STATUS_LOV_VW T3 ON T1.APPLICATIONSTATUSID = T3.APPLICATIONSTATUSID
LEFT JOIN DEV_DMA_VW.IPIPELINE_ORDERS_VW T4 ON T1.ORDERENTRYID = T4.ORDERENTRYID
LEFT JOIN DEV_USIG_STND_VW.AGMT_CMN_VW T5 ON T1.AGREEMENTID = T5.AGREEMENT_ID                    
LEFT JOIN DEV_USIG_STND_VW.PDCR_DEMOGRAPHICS_VW T8 ON T1.AGENTID = SUBSTR(TRIM(T8.BUSINESS_PARTNER_ID), CHARACTER_LENGTH(TRIM(T8.BUSINESS_PARTNER_ID)) - 5 FOR 6)
LEFT JOIN DEV_DMA_VW.FIRM_DIM_VW T9 ON T1.AGENCYNUMBER = T9.ORIGINALFIRMCODE
LEFT JOIN (SELECT DISTINCT T1.AGREEMENT_ID, SUM(TOTAL_DOLLAR_AMOUNT) AS "Deposit Amount"
                        FROM DEV_USIG_STND_VW.AGMT_PDCR_FIN_TXN_CMN_VW T1
                        INNER JOIN DEV_USIG_STND_VW.AGMT_CMN_VW T2 ON T1.AGREEMENT_ID = T2.AGREEMENT_ID
...
LEFT JOIN (SELECT AGREEMENT_ID, SUM(TRANSFER_AMOUNT) AS TRANSFER_AMOUNT FROM DEV_USIG_STND_VW.AGMT_REPLACEMENTS_CMN_VW GROUP BY 1) T11 ON T1.AGREEMENTID = T11.AGREEMENT_ID
LEFT JOIN (SELECT AGREEMENT_ID , MIN(TRANS_DT) AS FIRST_NIGO_DT, MAX(NIGO_RESOLUTION_DATE) AS NIGO_RES_DT
                        FROM DEV_USIG_STND_VW.AGMT_NIGO_CMN_VW GROUP BY 1) T12 ON T1.AGREEMENTID = T12.AGREEMENT_ID
...
From DEV_DMA_VW.DATE_DIM_VW ) as ReportDate on "Date" = ShortDate
