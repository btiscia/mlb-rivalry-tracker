...
, CAST(COALESCE(ORIGINALORDERSUBMITDATE,ORDERSUBMITDATE,NEWBUSINESSSUBMITDATE) AS DATE) AS SHORTDATE
, COALESCE(T3.PRODUCTTYPE,T2.MAJOR_PROD_NME) AS ProductType
, CASE WHEN To_Number (T1.AGENCYNUMBER) IS NULL THEN 'SDP' ELSE 'MMFA' END AS ChannelType
, SUM(COALESCE(TRANSFER_AMOUNT,TOTALINITIALPREMIUM)) AS Measure
FROM PROD_DMA_VW.ANB_APPLICATION_FCT_VW T1
LEFT JOIN PROD_USIG_STND_VW.AGMT_CMN_VW T2 ON T1.AGREEMENTID = T2.AGREEMENT_ID
LEFT JOIN PROD_DMA_VW.IPIPELINE_ORDERS_VW T3 ON T1.ORDERENTRYID = T3.ORDERENTRYID
LEFT JOIN PROD_USIG_STND_VW.AGMT_REPLACEMENTS_CMN_VW T5 ON T1.AGREEMENTID = T5.AGREEMENT_ID
GROUP BY 1,2,3,4
...
, COALESCE(T3.PRODUCTTYPE,T2.MAJOR_PROD_NME) AS ProductType
, CASE WHEN TO_NUMBER(T1.AGENCYNUMBER) IS NULL THEN 'SDP' ELSE 'MMFA' END AS ChannelType
, COUNT(COALESCE(ORIGINALORDERSUBMITDATE,ORDERSUBMITDATE,NEWBUSINESSSUBMITDATE)) AS Measure
FROM PROD_DMA_VW.ANB_APPLICATION_FCT_VW T1
LEFT JOIN PROD_USIG_STND_VW.AGMT_CMN_VW T2 ON T1.AGREEMENTID = T2.AGREEMENT_ID
LEFT JOIN PROD_DMA_VW.IPIPELINE_ORDERS_VW T3 ON T1.ORDERENTRYID = T3.ORDERENTRYID
LEFT JOIN PROD_USIG_STND_VW.AGMT_REPLACEMENTS_CMN_VW T5 ON T1.AGREEMENTID = T5.AGREEMENT_ID
