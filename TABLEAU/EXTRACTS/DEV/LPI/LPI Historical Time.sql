...
FROM (SELECT DISTINCT ShortDate "Date"
		    ...
		    INNER JOIN DEV_DMA_VW.DATE_DIM_VW T3 ON T2.DayOfWeek = T3.DayOfWeek AND ShortDate >= T1.StartDate
		    LEFT JOIN (SELECT PartyEmployeeID, CompletedDate, Sum(T1.ProductivityCredit) AS ProdCredits                     
							FROM DEV_DMA_VW.ACTIVITY_FCT_VW T1
							INNER JOIN DEV_DMA_VW.WORK_EVENT_CURR_DIM_VW T2 ON T1.WorkEventID = T2.WorkEventID
		    ...
		    LEFT JOIN DEV_DMA_VW.TIMEOUT_ACTIVITY_CURR_IVW T5 ON ShortDate = T5.MeetingDate AND T1.PartyEmployeeID = T5.PartyEmployeeID AND T5.MeetingDate >= T1.StartDate
