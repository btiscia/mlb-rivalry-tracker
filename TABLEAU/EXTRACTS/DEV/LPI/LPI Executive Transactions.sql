...
FROM (SELECT
		...
		FROM DEV_DMA_VW.TRANS_CURR_INTEGRATED_VW T1
...
LEFT JOIN (SELECT * FROM DEV_DMA_VW.GOAL_CURR_DIM_VW WHERE GoalTypeID = 5) T3 ON T1.FunctionID = T3.FunctionID AND T1.DepartmentID = T3.DepartmentID
