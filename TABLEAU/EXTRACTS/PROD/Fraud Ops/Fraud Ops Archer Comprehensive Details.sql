...
Author/Editor: Christina Valenti
...
,CASE
    WHEN ProductType = 'Insurance Products' AND ProductLine NOT LIKE ALL ('%Coverpath%', '%Haven%') THEN 'Insurance'
    WHEN ProductType = 'Wealth Management / Investment Products' THEN 'MMLIS'
	WHEN ProductType = 'Retirement Products' THEN 'Workplace'
    ELSE 'Other'
 END AS "Business Area"
...
,CASE WHEN ProductLine IS NULL THEN ProductType 
  ELSE ProductLine 
  END AS "Product Line"
...
,CASE 
	WHEN IncidentDisbmtAmountPaid <= IncidentDisbmtAmountRequested
				AND DateClosed IS NOT NULL THEN IncidentDisbmtAmountRequested - IncidentDisbmtAmountPaid ELSE 0 
END AS "Disbursement Amount Prevented"

 ,CASE
    WHEN CaseDisposition IN ('Fraud', 'Fraud Missed', 'Inconclusive - Suspected Fraud' )
                    AND DateClosed IS NOT NULL THEN 1 ELSE 0
END AS "Fraud Case Indicator"

 ,CASE
    WHEN CaseDisposition IN ('Fraud', 'Inconclusive - Suspected Fraud' ) 
                    AND (IncidentDisbmtAmountPaid = 0 OR IncidentDisbmtAmountPaid IS NULL)
                    AND DateClosed IS NOT NULL THEN 1 ELSE 0
...
END AS "Disbursement Requested Ind"
 ...
 ,CASE
    WHEN CaseDisposition IN ('Fraud', 'Fraud Missed', 'Inconclusive - Suspected Fraud' ) 
                    AND  (IncidentDisbmtAmountRequested > 0 AND IncidentDisbmtAmountPaid = 0)
                    AND DateClosed IS NOT NULL THEN 1 ELSE 0
...
FROM PROD_DMA_VW.ARCHER_CASE_DETAIL_VW
