...
Editor: Christina Valenti
Updated:  5/20/2022
Comments: Repointed to Vertica, replaced not like all statement  
*/
SELECT
	DISTINCT acv.AlertID AS "Alert ID" ,
    ...
    acv.IncidentTransactionCategory AS "Incident Transaction Category" ,
	--added the wording category
	...
	--taking product type from details view because case view sets it to other when prod line is null this logic should eventually move to the view.*/
	CASE
		WHEN acv.ProductLine IS NULL THEN acdv.ProductType
    ...
    CASE
		WHEN acv.ProductType = 'MULTIPLE' THEN 'MULTIPLE'
		WHEN acv.ProductType IS NULL
		AND acv.ProductLine = 'MULTIPLE' THEN 'MULTIPLE'
		WHEN acdv.ProductType = 'Insurance Products'
		--		AND acdv.ProductLine NOT LIKE ALL ('%Coverpath%','%Haven%') THEN 'Insurance'
		AND acdv.ProductLine NOT LIKE '%Coverpath%'
		AND acdv.ProductLine NOT LIKE '%Haven%' THEN 'Insurance'
	...
	CASE
		...
		AND acv.DateClosed IS NOT NULL THEN 1
		ELSE 0
	...
	CASE
		...
		OR acv.IncidentDisbmtAmountPaid IS NULL)
		AND acv.DateClosed IS NOT NULL THEN 1
		ELSE 0
	...
	CASE
		...
		AND acv.DateClosed IS NOT NULL THEN acv.IncidentDisbmtAmountRequested - acv.IncidentDisbmtAmountPaid
		ELSE 0
...
FROM
	dma_vw.archer_case_vw AS acv
LEFT JOIN 
    dma_vw.archer_case_detail_vw AS acdv ON
	acv.alertid = acdv.alertid
WHERE
	acv.DateClosed <= CURRENT_DATE
