...
, CASE WHEN T1.SOURCESYSTEMID = 35 THEN 'Home Office' ELSE 'SE2' END AS SLASource
, T3.FUNCTIONNAME
, GOALVALUE AS SLA
/*
...
,CASE WHEN T1.NewBusinessSubmitDate IS NOT NULL THEN COALESCE(T1.WithdrawnDate, T1.RejectDate, T1.IssueDate) END AS FinalDispositionDate
...
LEFT JOIN PROD_DMA_VW.GOAL_CURR_DIM_VW T3 ON T1.NEWBUSINESSDOCTYPE = T3.TransactionTypeName AND SLASource = FunctionName

QUALIFY ROW_NUMBER() OVER(PARTITION BY HOLDINGKEY, NEWBUSINESSDOCTYPE ORDER BY T1.TRANSDATE) = 1
