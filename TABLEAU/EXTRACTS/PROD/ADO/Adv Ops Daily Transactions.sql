...
*  Peer Review Date: 
*  Source for this routine is PROD_DMA_VW.ACT_ADO_PIT_INTEGRATED_VW, PROD_DMA_VW.DATE_DIM_VW 
...
*Revised: 7/9/2020 - Removed holiday field and removed left join to date dim to improve query efficiency -KC
*Revised: 11/2/2020 - Removed case statement for productivity credits as it is no longer needed -KC */
...
--, Count(IsHoliday) as Holidays 	--Removed 7/9 KC
,TAT
...
--          END AS "Productivity Credits" removed 11/2/2020 as issue was resolved on data side statement no longer necessary
,NIGODescription
...
--LEFT JOIN PROD_DMA_VW.DATE_DIM_VW T2 ON T2.ShortDate BETWEEN "Date" AND T1.ExpectedCompletedDate AND IsHoliday = 1	--Removed to improve query efficiency KC
WHERE (WorkEventDepartmentID = 48 OR DepartmentID = 48)
