/*
FILENAME: Adv Ops HISTORICAL DETAILS CURRENT
CREATED BY: Bill Trombley
LAST UPDATED: 01/20/2022
CHANGES MADE: Vertica Migration.
LAST UPDATED:  03/08/2023
    CASE STATEMENT ADDED FOR bcc_ind to show Y or N instead of 1,0.  LC
*/
...
SELECT
	...
	, CASE WHEN T1.completed_dt IS NOT NULL THEN 1 ELSE 0 END AS "Completed Indicator"
	, T1.expected_completed_dt AS "Follow Up Date"
	, T1.Firm
    , T1.firm_display_nm AS "Firm Name"
	, T1.Region
	, T1.rqstr_des AS "Requestor Type Name"
	, goal_val AS "IGO Goal"	
FROM dma_vw.fact_integrated_ado_curr_vw T1
LEFT OUTER JOIN (SELECT goal_val, department_id, function_id FROM dma.dma_dim_goal_curr WHERE end_dt = '9999-12-31' AND goal_type_id = 5) T2 
ON T1.work_event_function_id = T2.function_id AND T1.employee_department_id = T2.department_id
WHERE (T1.trans_type_id IN (1,3) OR CAST(T1.logged_dt AS DATE)>= (Add_Months(CURRENT_DATE(), -36)))