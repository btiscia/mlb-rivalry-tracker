...
, CASE  when left(lower(T1.mmid),2) in ('rp', 'rb')
                or T1.role_grade_id = 14
            then 1
            else 0
            end AS "Bot Indicator"
, CASE  when (T1.party_type_id = 2
                  or (Coalesce(T1.party_type_id, -99) = -99
                        and (left(lower(T1.mmid),2) in ('mm', 'ct', 'ot')
                                or chnl_dspy_nm in ('CSS', 'Mobile', 'System', 'ServiceNet', 'ServiceNet2'))))
                 and left(lower(T1.mmid),2) not in ('rp', 'rb')    --non-bot system account
                 and T1.role_grade_id <> 14
            then 1
            else 0
            end AS "System Account Indicator"
, CASE  when (T1.employee_department_id = 51
                  or (COALESCE(T1.employee_department_id, -99) = -99
                         and lower(T1.mmid) like 'ot%'))
                  and T1.party_type_id <> 2
            then 1
            else 0
            end AS "Hyderabad Employee Indicator"
  , CASE  when (T1.employee_department_id NOT IN (51, -99)
                or (COALESCE(T1.employee_department_id, -99) = -99
                    and left(lower(T1.mmid),2) in ('mm', 'ct')))
                and T1.party_type_id <> 2
            then 1
            else 0
            end AS "US Employee Indicator"
, CASE
        when left(lower(T1.mmid),2) in ('rp', 'rb')
            or T1.role_grade_id = 14
        then 'Bot'
        when T1.party_type_id = 2
            or (Coalesce(T1.party_type_id, -99) = -99
                and (left(lower(T1.mmid),2) not in ('mm', 'ct', 'ot')
                        or T1.chnl_dspy_nm in ('CSS', 'Mobile', 'System', 'ServiceNet', 'ServiceNet2')))
        then 'System Acct'
...
, coalesce(tmp_dig_adopt.processing_type_nm, 'unknown') as "Processing Type Name"
...
LEFT OUTER JOIN
tmp_dig_adopt
ON tmp_dig_adopt.source_transaction_id = T1.source_transaction_id
 /* Logged -by employee info */
LEFT OUTER JOIN
dma_vw.dma_dim_employee_pit_vw AS T4
ON T1.logged_by_team_party_id = T4.team_party_id
 /* Get  IGO Goal*/
LEFT OUTER JOIN
(SELECT *  FROM dma.dma_dim_goal_curr WHERE goal_type_id = 5) AS T2
ON T1.work_event_function_id = T2.function_id
AND T1.employee_department_id = T2.department_id
WHERE
T1.trans_type_id = 3
AND CAST(T1.long_completed_dt AS Date) >= (Add_Months(CURRENT_DATE(), -36))  ---when going to 5 years then change -36 to -60
--AND CAST(T1.long_completed_dt AS Date) >= '2022-01-01'
AND T1.work_event_department_id= 5
AND lower(work_event_nm) not like '%flags/blockers%'
AND lower(work_event_function_nm) not like '%flags/blockers%'
AND lower(work_event_segment_nm) not like '%flags/blockers%'
AND lower(work_event_primary_role_nm) not like '%flags/blockers%'
AND work_event_num <> 4763      -- OPERATIONS RECOVERY
--Based on conversation between Dan and Brian it was decided to not filter these event ids out.  8/3/2023
--AND work_event_id not in (28506, 28403, 28404)        --LISR agent notification 28403 and 28404, CA Endorsement-28506