/*
FILENAME: PSC HISTORICAL DETAILS CURRENT
...
/*
 ...
 LEFT JOIN(SELECT DISTINCT
				T2.source_transaction_id AS SourceTrans
				,FIRST_VALUE(T2.completed_dt) OVER(PARTITION BY T2.source_transaction_id
				ORDER BY T2.load_dt DESC) AS CompDate
	FROM dma_vw.fact_integrated_psc_vw T2
	WHERE T2.completed_dt IS NOT NULL
	AND T2.trans_type_id = 3
	AND T2.completed_dt < CURRENT_DATE() ) AS CompRec
ON CompRec.SourceTrans = T1.source_transaction_id
*/
