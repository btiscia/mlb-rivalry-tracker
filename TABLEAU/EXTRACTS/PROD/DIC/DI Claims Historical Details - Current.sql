...
SELECT
	...
	, CASE WHEN T1.trans_type_id = 2 THEN COALESCE(T1.demand_credit,T1.current_prod_credit) ELSE T1.current_prod_credit END AS "Productivity Credits"	
	, T1.tat AS "TAT" 	
	...
	, T1.work_event_met_expected_ind AS "Met Expected Ind Count"
	...
	, CASE WHEN upper(T1.work_event_system_nm) = 'CATS'THEN 'CATS'
		   ...
		   WHEN upper(T1.work_event_system_nm) = 'MEDVOC'THEN 'MEDVOC'	
	  ELSE 'UNKNOWN' END AS "Processing System"		
,COALESCE(T1.log_employee_last_nm || ', ' || T1.log_employee_first_nm, 'Unknown') AS "Logged by Employee" 
